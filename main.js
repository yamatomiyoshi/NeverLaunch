...
function draw() {
    ...
    if (gamestate == START) {
        ctx.fillStyle = "black"
        ctx.beginPath();
        ctx.fillText("画面をクリックすると、赤の国の王様（赤い点）と青の国の王様（青い点）の最終決戦がはじまります。プレイヤーはワープ（黒い四角）を出現させて、2人をワープさせて移動させることで、衝突を阻止し続けてください。nワープはワープの素である飲料ワーピル（緑の四角）を獲得することで、使用できるようになります。より長い時間衝突を回避し続けることで、スコアが加算されます画面左上に表示されます。ワーピルを適宜補給しながら2人の衝突を回避し続け、ハイスコアを目指しましょう。", 100, 100);
    }
    else if (gamestate == PLAYING) {
        //赤い敵
        ctx.fillStyle = "red";
